    ...
    while (1) {
        ...
        if (op == "quit" || op == "exit") {
            ...
            if (key == "ISBN") {
                ...
                key_valid = ChineseValidator(val).max_len(60).han().no_quotes().no_pipes();
            }
            else {
                key_valid = ChineseValidator(val).max_len(60).han().no_quotes();
            }
            if (!key_valid) {
                std::cout << "Invalid\n";
                continue;
            ...
            for (int i = 1; i < tokens.size(); i++) {
                ...
                if (key == "ISBN") {
                    ...
                    if (!ChineseValidator(val).max_len(60).han().no_quotes()) {
                        parse_success = false;
                        break;
                    }
                    if (key == "keyword") {
